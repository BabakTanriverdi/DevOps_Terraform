...
resource "aws_instance" "tf-instances" {
  ami = "ami-04b70fa74e45c3917"           # actually ami from AWS EC2
  instance_type = "t3.micro"              # actually type from instance
  key_name = "my-key-name"            # change here
  tags = {
    Name = "instance name"            # actually EC2 name
  }
}
```
...
- Import the EC2 instance. (Take the EC2 instance-id from the console)

```bash
terraform import "aws_instance.tf-instances" i-092fe70d1cef163c1 #(instance ID)
...
```bash
terraform state rm aws_security_group.my-sec-grp
```
